...
class ExactGPModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood):
        ...
        self.covar_module = gpytorch.kernels.SpectralMixtureKernel(num_mixtures = n_gaussians_Q, ard_num_dims = train_x.shape[1])
...
likelihood = gpytorch.likelihoods.GaussianLikelihood(noise_constraint=gpytorch.constraints.Interval(0.1, 0.8))
model = ExactGPModel(train_x, train_y, likelihood)

#model.covar_module.mixture_means = torch.tensor([[[0.0030, 0.1583]]])
#model.covar_module.mixture_scales = torch.tensor([[[0.1023, 0.0179]]])
#model.covar_module.mixture_weights = torch.tensor([6.5732])


# saving hyperparameters:
path = '../logs/f5/SM_'+str(n_gaussians_Q)+'gauss_.txt'
