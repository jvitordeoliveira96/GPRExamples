...
class MultitaskGPModel(gpytorch.models.ExactGP):
    ...
    def reset(self):
        self.covar_module.data_covar_module.lengthscale = (
            torch.rand(self.covar_module.data_covar_module.lengthscale.size()
            )
        )
        # To do: define restart for task kernel
        # self.covar_module.task_covar_module
...
path = '../logs/f6/RBF_Multitask.txt'
if(not os.path.exists(path)):
    os.makedirs(path)
output_f = open(path, 'w')

output_f.write('hyperparameters (initial guess)\n')
output_f.write('Data Kernel, Lengthscale: \n')
output_f.write( str(model.covar_module.data_covar_module.lengthscale) + '\n')
