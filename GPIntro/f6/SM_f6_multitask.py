...
class MultitaskGPModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood):
        ...
        self.covar_module = gpytorch.kernels.MultitaskKernel(
            gpytorch.kernels.SpectralMixtureKernel(num_mixtures = n_gaussians_Q), num_tasks=2, rank=1
        )
        self.covar_module.data_covar_module.initialize_from_data(train_x, train_y)
    ...
    def reset(self):
        ...
        self.covar_module.task_covar_module.var = (
            torch.rand(self.covar_module.task_covar_module.var.size()
            )
        )
...
path = '../logs/f6/SM_{0}gauss_Multitask.txt'.format(n_gaussians_Q)
if(not os.path.exists(path)):
    os.makedirs(path)
output_f = open(path, 'w')

output_f.write('hyperparameters (initial guess)\n')
output_f.write('Data Kernel, Mean: \n')
output_f.write( str(model.covar_module.data_covar_module.mixture_means) + '\n')
output_f.write('Data Kernel, Variances (lengthscales): \n')
output_f.write( str(model.covar_module.data_covar_module.mixture_scales) + '\n')
output_f.write('Data Kernel, Weights: \n')
output_f.write( str(model.covar_module.data_covar_module.mixture_weights) + '\n')
output_f.write('Task Kernel, Var (v): \n')
output_f.write( str(model.covar_module.task_covar_module.var) + '\n')
output_f.write('Task Kernel, Covar factor (B): \n')
output_f.write( str(model.covar_module.task_covar_module.covar_factor) )
...
output_f.write('Task Kernel, Covar factor (B): \n')
output_f.write( str(model.covar_module.task_covar_module.covar_factor) + '\n')
output_f.write('Likelihood, Noise: \n')
output_f.write( str(likelihood.noise ) )
output_f.close()
...
plt.title('SM Kernel {0} Gaussians'.format(n_gaussians_Q))
# Set xlim
plt.xlim([-3.5, 3.5])
# Set ylim
plt.ylim([-3, 3])
